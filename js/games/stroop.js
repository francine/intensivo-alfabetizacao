...
(function () {
  ...
  register({
    ...
    start(stage, api) {
      const total = 18;
      const CELEBRATE_AT = 3; // celebra a cada N seguidos
      let t = 0, correct = 0, streak = 0, bestStreak = 0, curInk, tick, tLeft, wordEl;
      ...
      api.intro("🎨", "Cores Trocadas",
        ...
        () => { t = 0; correct = 0; streak = 0; bestStreak = 0; next(); });

      function stats() { api.setStats({ Certos: correct, Sequência: streak, Rodada: `${t}/${total}` }); }

      function next() {
        clearInterval(tick);
        if (t >= total) {
          const score = Math.round((correct / total) * 100);
          return api.finish({ score, max: 100, scoreText: `${correct}/${total} certos · melhor sequência ${bestStreak}`, note: score >= 80 ? "Foco de aço sob conflito!" : "Ignorar a leitura automática é difícil — bom treino executivo." });
        }
        t++;
        ...
        COLORS.forEach((c) => {
          btns.appendChild(h.el("button", {
            class: "btn", style: { background: c.hex, color: "#fff", minWidth: "92px", minHeight: "48px", boxShadow: "0 6px 14px " + c.hex + "55" },
            text: c.name.charAt(0) + c.name.slice(1).toLowerCase(),
            onclick: () => answer(c.name === curInk.name, fb, btns),
          }));
        });

        wordEl = h.el("div", { style: { display: "inline-block", fontSize: "3.4rem", fontFamily: "var(--font-display)", fontWeight: 700, color: curInk.hex, margin: "16px 0", letterSpacing: ".02em", maxWidth: "100%", wordBreak: "break-word" }, text: word.name });
      ...
      function celebrate(el) { el.classList.remove("g-celebrate"); void el.offsetWidth; el.classList.add("g-celebrate"); }
      ...
      function answer(ok, fb, btns, timeout) {
        ...
        if (ok) {
          correct++; streak++; bestStreak = Math.max(bestStreak, streak); sfx.good();
          if (streak > 0 && streak % CELEBRATE_AT === 0) {
            sfx.win(); celebrate(wordEl);
            fb.textContent = `Sequência de ${streak}! 🔥`;
          } else { fb.textContent = "Certo! ✅"; }
          fb.className = "g-feedback ok";
        } else {
          streak = 0; sfx.bad();
          wordEl.classList.remove("g-wrong"); void wordEl.offsetWidth; wordEl.classList.add("g-wrong");
          fb.textContent = timeout ? "Tempo! ⏱️ era " + curInk.name : "Era " + curInk.name; fb.className = "g-feedback no";
        }
        stats();
        setTimeout(next, 750);
      }
